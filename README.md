...
|[phm-shinonome](#phm-shinonome)| 3 | 14725 |ISO 8859-1 + All latin-based MS-DOS and Windows codepages|Japanese CP932 + Jōyō + Jinmeiyō + CP737 + CP869 + CP1253 (Greek) + CP855 + CP866 + CP1251 (Cyrillic)|CP437 + Emojis + more|
...
|[phm-beyondneo-*](#phm-beyond-neo-figfont-family)| 5 | 4020 |ISO 8859-1 + CP1252|Japanese Katakana + a few Kanji|CP437 + Some emojis|
...
This is my original CP437-compatible ANSI/VT-based font, containing over 500 characters.
...
**Modern additions**
- More Latin letters (Latin-1 Supplement, Latin Extended-A/B/C/D/E, IPA Extensions, Phonetic Extensions, Latin Extended Additional blocks)
...
- Emojis (1506 emojis, all Unicode 17.0 Emojis AFAIK + a few from upcoming Unicode 18.0)

With 14725 characters, phm‑shinonome is probably the largest FIGlet font ever created, unifying Japanese, Western, and semigraphic bitmap lineages into a single Unicode‑coherent design, with far broader Unicode coverage than any prior FIGlet font.
...
It uses just 5 rows by taking advantage of Unicode 16.0 Symbols for Legacy Computing. It contains 4020 characters, and supports the ASCII, ISO 8859-1 and Windows 1252 Latin-1 character sets, Greek, Cyrillic, Japanese Katakana (and Hiragana as Katakana), 152 ideographs (mostly for dates, times, places, and numbers in Chinese numerals), 495 color symbols and emojis (all smiley faces, hands, and hearts, as well as some pictographs and emojis), and more.
...
### Installing FIGlet fonts
You'll need the FIGlet utility, so start with `sudo apt install figlet` or equivalent for your distro.
FIGfonts can be used as is by specifying their paths:
```bash
echo 'Hello, world!' | figlet -f /some_path/phm-largetype.flf -C utf8
```
The `-C utf8` option makes FIGlet decode UTF-8 input into Unicode code points and use those code points directly as FIGlet character indices, enabling characters beyond ASCII.
...
-- Philippe Majerus, October 2024 to August 2026
